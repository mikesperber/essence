...
\item \var{Input} is a \link{stream}{sec:streams} (as per the
  \codefont{stream} structure) consisting of enumerands of the
  terminals of \var{grammar} paired with their attribute values.  It is the
  actual input to the parser. 
  
  The stream actually contains the \emph{ordinal numbers} of the enumerated
...
\link{streams}[ (see Sec.~\ref{sec:streams})]{sec:streams} as well as
...
distribution as \filename{common/stream.scm}.  It is also the
implementation of the Scheme~48 structure \codefont{stream} defined in
\filename{packages.scm}.  Aside from the accessors described
\link{above}{sec:streams-accessors}, it also provides a creation
...
As indicated in Sec.~\ref{sec:streams}, the input to the parser comes in form
of a stream. To open the stream module type
\begin{verbatim}
> ,open stream
\end{verbatim}
